...
# second hand-written list in this repository. AGDA_SOURCE_DIR may point at an
# unpacked Agda checkout; otherwise use Cabal's local source cache.
...
cleanup() {
  rm -f "${temporary_path}" "${temporary_path}.out"
}
trap cleanup EXIT

# Both enum declarations put their first constructor after '=' and subsequent
# constructors after '|'. Count both forms so the inventory is exhaustive.
awk '
...
while IFS=$'\t' read -r kind constructor; do
  agda_name="$(printf '%s' "${constructor}" | awk '{ printf "%s%s", tolower(substr($0, 1, 1)), substr($0, 2) }')"
